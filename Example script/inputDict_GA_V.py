input_dict = {
    ...
    "height_dimension": 0,          	  # Dimension corresponding with the height of the electrode (sides of the electrode)
    "length_dimension": 1,              # Dimension corresponding with the length of the electrode (flow dimension)
    "width_dimension": 2,               # Dimension corresponding with the thickness of the electrode (current collector <-> membrane)
    
    # Phases
    "anolyte_conductivity": 28,         # Anolyte electrical conductivity [S m-1]
    "anolyte_density": 992,             # Anolyte density [kg m-3]
    "anolyte_viscosity": 8.9e-4,        # Anolyte viscosity [Pa s] 
    ...
    "catholyte_conductivity": 28,       # Anolyte electrical conductivity [S m-1]
    "catholyte_density": 992,           # Anolyte density [kg m-3]
    "catholyte_viscosity": 8.9e-4,      # Anolyte viscosity [Pa s] 
    ...
    "D_a": 2.11e-10,                    # Anolyte active species diffusivity (Fe2+) [m2 s-1]
    ...
    "j0_a": 0.39,                       # Anolyte exchange current density [A m-2]
    "val_a": 1,                         # Amount of valence electrons participating in the half-reaction [-]
    "alpha_a_a": 0.42,                  # Anodic transfer coefficient for the half-reaction in the anode [-]
    "alpha_c_a": 0.42,                  # Cathodic transfer coefficient for the half-reaction in the anode [-]
    
    "D_c": 2.11e-10,                    # Catholyte active species diffusivity (Fe3+) [m2 s-1]
    ...
    "j0_c": 0.39,                       # Catholyte exchange current density [A m-2]
    "val_c": 1,                         # Amount of valence electrons participating in the half-reaction [-]
    "alpha_a_c": 0.42,                  # Anodic transfer coefficient for the half-reaction in the cathode [-]
    "alpha_c_c": 0.42,                  # Cathodic transfer coefficient for the half-reaction in the cathode [-]
    ...
    "V_step": -0.5,                     # Step change in the cell voltage [V] 
    "E_cell_final": -0.5,               # Final value of the cell voltage range [V] # (E_cell < 0 --> Charging, E_cell > 0 --> Discharging)
    
    # Membrane properties
    "membrane_resistivity": 0.16/1e4,   # Membrane ionic resistivity [Ohm m2]
    ...
    # Numerical settings
    "rel_tol": 1e-3,                    # Relative tolerance between the solutions for two sequential iterations
    "abs_tol": 1e-3,                    # Absolute tolerance between the estimated external current density between anodic and cathodic side
    "max_iter": 5000,                   # Maximum amount of iterations
