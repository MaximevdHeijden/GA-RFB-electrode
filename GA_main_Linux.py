...
import GA_Functions_Linux as func
...
eta_c = -0.25                      # Cathodic overpotential guess
...
ReloadData = 0
# List the amount of cores used:
cores = population_size
...
if ReloadData == 0:
    ...
    print('Checking overall pore volume')
...
def perform_simulation(arg):
    global eta_c
    global eta_a

    i = arg
    # Run the algorithm for the network and network copy
    P_pump_ideal, P_el, P_el_ideal, P_theory, current_sum, eta_c_tmp, eta_a_tmp = func.algorithm(net[i], net_copy[i], eta_c, eta_a, generation, Extracted, FF, ReloadData, reload, inputDict, cf, i)      
    eta_c = eta_c_tmp
    eta_a = eta_a_tmp
    # Determine the fitness of the network
    fitness = func.determine_fitness(P_pump_ideal, P_el, P_el_ideal, P_theory)
    surface = net[i]['pore.surface_area'][net[i].pores('internal')].sum()
    current = current_sum
    pumping = P_pump_ideal        

    return i, fitness, surface, current, pumping
...
for generation in range(num_start, num_generations):
    ...
    with Pool(processes=cores) as pool:
        for (i, fitness_i, surface_i, current_i, pumping_i) in pool.imap(perform_simulation, range(len(net))):
            fitness[generation, i] = fitness_i
            current[generation, i] = current_i
            surface[generation, i] = surface_i
            pumping[generation, i] = pumping_i

    # Display the best result:
    print(f"Highest achieved fitness in generation {generation} is {fitness[generation, :].max()}")
    print(f"Fitness mean:", fitness[generation, :].mean())
    ...
    if M_and_S == 1:
      for j, _ in enumerate(parent_projects):
            os.remove('./Genetic_algorithm' + '/' + folder + "/j0_1_generation_" + str(generation) + "_parent_" + str(j) + ".pnm")      

    if generation == 0 or generation == 1 or generation%50 == 0 or generation == (num_generations -1):  
        # Data storage
